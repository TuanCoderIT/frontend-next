...
const RatingForm = ({ type, id, onRatingSubmitted }: RatingFormProps) => {
    ...
    const handleDelete = async () => {
        ...
        if (!confirm("Bạn có chắc chắn muốn xóa đánh giá của bạn?")) return;
        ...
        try {
            ...
            setError(err.response?.data?.message || "Lỗi khi xóa đánh giá. Vui lòng thử lại.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const getRatingText = () => {
        if (stars === 0) return "Chọn đánh giá";
        if (stars === 1) return "Kém";
        if (stars === 2) return "Trung bình";
        if (stars === 3) return "Tốt";
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
                <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 mb-4">
                        Vui lòng đăng nhập để chia sẻ đánh giá và phản hồi của bạn
                    </p>
                    <a
                        href="/auth/login"
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            ...
            {showSuccess && (
                <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg flex items-center">
                    ...
                    <span className="text-green-800 text-sm">
                        {myRating && !isEditing ? "Đánh giá đã được cập nhật thành công!" : "Đánh giá đã được gửi thành công!"}
                    </span>
                </div>
            )}
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h3 className="text-lg font-semibold text-gray-900 flex items-center">
                        ...
                        {myRating && !isEditing ? "Đánh giá của bạn" : "Đánh giá bài kiểm tra này"}
                    </h3>
                    {myRating && !isEditing && (
                        <p className="text-sm text-gray-600 mt-1">
                            Bạn đánh giá bài kiểm tra này vào {new Date(myRating.created_at || Date.now()).toLocaleDateString()}
                        </p>
                    )}
                </div>
            ...
            <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                    Chia sẻ suy nghĩ của bạn (tùy chọn)
                </label>
                <textarea
                    value={comment}
                    onChange={(e) => setComment(e.target.value)}
                    placeholder="Nói cho chúng tôi biết suy nghĩ của bạn về bài kiểm tra này..."
                    rows={4}
                ...
                >
                    {isLoading ? (
                        <>
                            ...
                            {myRating && !isEditing ? "Đang cập nhật..." : "Đang gửi..."}
                        </>
                    ) : (
                        <>
                            <Send className="w-4 h-4 mr-2" />
                            {myRating && !isEditing ? "Cập nhật đánh giá" : "Gửi đánh giá"}
                        </>
                    )}
                </button>
            </div>
